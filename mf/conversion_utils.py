...
def convert_all(model_path: str, checkpoint_path: str, half: bool):
...
def convert_vae(model_path: str, checkpoint_path: str):

    assert model_path is not None, "Must provide a model path!"

    assert checkpoint_path is not None, "Must provide a checkpoint path!"

    vae_path = osp.join(model_path, "diffusion_pytorch_model.bin")

    # Convert the VAE model
    vae_state_dict = torch.load(vae_path, map_location="cpu")
    vae_state_dict = convert_vae_state_dict(vae_state_dict)
    vae_state_dict = {k: v for k, v in vae_state_dict.items()}

    # Put together new checkpoint
    state_dict = {"state_dict": vae_state_dict}
    torch.save(state_dict, checkpoint_path)